...
export const orders: OrderItem[] = [
  {
    ...
    flowbackPlan: [
      { label: "更新规格核对培训内容", owner: "培训运营", status: "pending", note: "把规格核对流程写入学习课件和审单规范。" },
    ...
    annotation: {
      ...
      syncTargets: ["培训运营", "信息同步中心", "销售-设计协同"],
      shouldFlowback: true,
    },
    validation: {
      overallStatus: "risk",
    ...
    afterSaleAttribution: {
      ...
      syncTargets: ["信息同步中心", "培训运营", "销设协同"],
    },
    trainingFlowback: {
      summary: "这张单需要把“规格核对”从经验动作升级成标准动作，同时让培训、题库和 AI 陪练都能覆盖到。",
      standardGap: "目前缺少“设计图版本必须写入订单”的硬约束，也缺少专门训练销售识别规格冲突的内容。",
      targets: [
        { type: "课程", label: "《规格核对与图纸版本确认》补训课", owner: "培训运营", status: "pending", note: "补齐版本确认和规格核对动作。", due: "今天 18:00 前" },
  ...
  },
  {
    ...
    taskType: "工艺版本未同步",
    ownerView: "信息同步",
    nextActions: ["立即同步售后和门店，停止继续使用旧材料口径", "追溯客户沟通记录，看是否沿用旧版施工解释", "将禁用材料案例回流到信息同步与培训运营"],
    rootCauses: [
      { label: "知识版本同步", owner: "信息同步", status: "confirmed", note: "门店仍有人员未确认 v3.1，导致继续使用旧口径。" },
      { label: "题库 / 课件", owner: "培训运营", status: "risk", note: "题库和陪练场景仍沿用旧厚度与材料解释。" },
      { label: "售后归因标准", owner: "售后回流", status: "pending", note: "需补充售后记录模板，明确知识版本字段。" },
    ],
    flowbackPlan: [
      { label: "触发工艺规范重学", owner: "培训运营", status: "pending", note: "覆盖已学习旧版规范的相关人员。" },
    ...
    afterSaleAttribution: {
      ...
      syncTargets: ["信息同步中心", "培训运营", "售后模板"],
    },
    trainingFlowback: {
      summary: "回流重点不只是更新一条课件，而是让“版本确认 -> 现场解释 -> 售后记录”形成同一套标准。",
      standardGap: "目前培训、题库、陪练和售后模板之间没有共用的知识版本字段，导致回流后仍难闭环。",
      targets: [
        { type: "课程", label: "《防水规范 v3.1》补学任务", owner: "培训运营", status: "pending", note: "覆盖仍未确认新版规范的人员。", due: "今天 17:00 前" },
  ...
  },
  {
    ...
    rootCauses: [
      ...
      { label: "讲解训练", owner: "培训运营", status: "risk", note: "陪练场景没有覆盖湿区等级选择。" },
      { label: "安装核对", owner: "安装交付", status: "pending", note: "安装前未再次核对产品适用区间。" },
    ],
    flowbackPlan: [
      { label: "补充防滑等级课程案例", owner: "培训运营", status: "pending", note: "把湿区 / 干区区分讲清楚。" },
    ...
    validation: {
      ...
      recommendations: ["把湿区等级判断加入陪练和题库。", "补一条安装前核对动作。", "在带教看板里跟进该人员复盘结果。"],
    },
    afterSaleAttribution: {
      ...
      syncTargets: ["带教看板", "培训运营", "售后归因"],
    },
    trainingFlowback: {
      summary: "这条回流要同时进入课程、题库和带教任务，避免参数理解偏差只停留在售后复盘。",
      standardGap: "目前参数类训练偏重名词记忆，缺少“真实使用场景 -> 正确推荐 -> 现场核对”的完整链路。",
      targets: [
        { type: "课程", label: "《防滑等级与应用场景》案例补充", owner: "培训运营", status: "pending", note: "补强湿区 / 干区判断。", due: "明天下午" },
